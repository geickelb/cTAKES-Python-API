...
class cTAKES(NLPProcessing):
    def __init__(self, object):
        ...
        # print('(ctakes.py) Object being used:', object)
        super().__init__()

        self.name = 'ctakes'
        self.bin = os.path.dirname(os.path.abspath('%s/../' % inspect.getfile(inspect.currentframe())))
        self.root = object['root']
        self.cleanup = object['cleanup']

        # print('(ctakes.py) Self name:', self.name)
        # print('(ctakes.py) Self bin:', self.bin)
        # print('(ctakes.py) Self root:', self.root)
        # print('(ctakes.py) Self cleanup:', self.cleanup, '\n')
    ...
    def _run(self):
        if call(['{}/cTAKES-Python-API/ctakes-install/run.sh'.format(self.bin)]) > 0:
    ...
    def _write_input(self, text, filename):
        ...
        in_dir = os.path.join(
            self.root if self.root is not None else '.', 'ctakes_input')
        if not os.path.exists(in_dir):
            logging.error(
                "The input directory for cTAKES at %s does not exist" % in_dir)
            return False
    ...
    def _parse_output(self, filename, **kwargs):
        ...
        if not os.path.exists(out_dir):
            logging.error(
                "The output directory for cTAKES at %s does not exist" % out_dir)
            return None
        ...
        code_nodes = root.getElementsByTagNameNS(
...
if '__main__' == __name__:
    ...
    ### Defines directory to run 
    # print('(ctakes.py) creating directory_obj')
    directory_obj = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ctakes-test')
    # print('(ctakes.py) directory_obj = ', directory_obj)

	### Starts cTAKES class defining root directory as run_dir & cleanup as True
    # print('(ctakes.py) Starting my_ctakes')
    my_ctakes = cTAKES({'root': directory_obj, 'cleanup': True})

    my_ctakes.prepare()

	# create a test input file
    with open(os.path.join(my_ctakes.root, 'ctakes_input/test.txt'), 'w') as handle:
